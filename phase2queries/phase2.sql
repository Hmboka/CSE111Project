...
--------------   4   --------------
--insert into fav author #WORKS
INSERT INTO faveauthors (fa_name, fa_myrating)
VALUES ('HabiTesting', 2.4);


--------------   5   --------------
--insert into hasread from books table #WORSK (forgot rating input, also best to use bookID instead of title, left title for demonstration purposes)
...
WHERE b_bookID = 22;
    ...
    --------------   6   --------------
    --sample to show the next query #WORKS
                    INSERT INTO hasread (hr_bookID, hr_name, hr_myrating)
...
--------------   7   --------------
--Delete books from hasread #WORKS
DELETE FROM hasread
...
--------------   8   --------------
--retroactively add your own rating to books added to hasread #WORKS
UPDATE hasread
SET hr_myrating = 4.2
WHERE hr_bookID = 77;
...
--------------   9   --------------
--(TRY NOT TO RUN QUERY BC ONCE A BOOK IS DELETED WE MAY NOT GET IT BACK) delete books fom the main book able #WORKS
DELETE FROM books
WHERE b_bookID = 1;


--------------   10   --------------
--delete books based on the author WORKS
DELETE FROM books
where ad_name = 'testingtesting123'


--------------   11   --------------
--find books and author written by blank author that start with The #WORKS
...
--------------   12   --------------
--find favorite books which are written by a specific author #WORKS
SELECT fb_title
FROM favebooks
...
--------------   15   --------------
--find books that were published after 2002 and are on your has read list (table) #WORKS I THINK
SELECT hr_name 
FROM hasread
...
--------------   17   --------------
--what book has a rating of at least 4.0, written in english, and has at least 600 pages #WORKS
...
--------------   18   --------------
--find faveauthor(s) with highest rating #WORKS
